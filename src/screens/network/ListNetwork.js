...
class ListNetwork extends Component {
  ...
  async componentDidMount() {
    ...
    let a = await listPmNetwork(db);
    this.setState({networks: a});
    this._unsubscribe = this.props.navigation.addListener('focus', async () => {
      a = await listPmNetwork(db);
      this.setState({networks: a});
      console.log(db);
    });
  }
      ...
      <Pressable
        onPress={() => {
          this.props.navigation.navigate('ViewNetwork', {
            id: item.pm_network_device_id,
          });
        }}>
        <DataList
          item_1={item.tanggal}
          item_2={item.nama_perangkat}
  ...
  writeCsv = async () => {
    try {
      ...
      if (granted === PermissionsAndroid.RESULTS.GRANTED) {
        ...
        // construct csvString
        const headerString = 'Tanggal;Nama Teknisi;Nama Perangkat\n';
        const rowString = networks
          .map(d => `${d.tanggal};${d.nama_teknisi};${d.nama_perangkat}\n`)
          .join('');
        const csvString = `${headerString}${rowString}`;

        // write the current list of answers to a local csv file
        const pathToWrite = `${RNFetchBlob.fs.dirs.DownloadDir}/Network-${thisMonth}.csv`;
        console.log('pathToWrite', pathToWrite);
        RNFetchBlob.fs
          .writeFile(pathToWrite, csvString, 'utf8' /*csvString, 'utf8'*/)
          .then(() => {
            Toast.show({
              title: `Data berhasil di-download ${pathToWrite}`,
              status: 'success',
            });
  ...
  render() {
    let buttonDownload;
    const data = this.state.networks;
    if (data != 0) {
      buttonDownload = (
